...
# * recode apply and filtering operations with dplyr
...
option_list <- list(
  ...
  make_option(c("-r", "--reference-ids"), type="character", default=NULL,
              ...
              dest="refIDs"),
  make_option(c("-a", "--xaxis_label"), type="character", default="Target",
              help="label for X-axis [default %default]",
              dest="x_label"),
  make_option(c("-b", "--yaxis_label"), type="character", default="Query",
...
#            keep_ref=23, 
#            similarity=T, h_lines=T, similarity=T, interactive=F, plot_size=15, on_target = T, v=FALSE)
...
if(opt$v){
  ...
  cat(paste0("label for x-axis (-a): ", opt$x_label,"\n"))
  cat(paste0("label for y-axis (-b): ", opt$y_label,"\n"))
}

# commented out remove path from file name in order to save in a folder
# opt$output_filename = unlist(strsplit(opt$output_filename, "/"))\
#	[length(unlist(strsplit(opt$output_filename, "/")))]
...
alignments <- alignments %>%
	mutate(percentID = numResidueMatches / lenAln)
# alignments$percentID = alignments$numResidueMatches / alignments$lenAln

# Flip starts, ends for negative strand alignments
alignments <- alignments %>%
  mutate(queryStartTemp = queryStart) %>%
  mutate(queryStart = ifelse(strand == "-", queryEnd, queryStart)) %>%
  mutate(queryEnd = ifelse(strand == "-", queryStartTemp, queryEnd)) %>%
  select(-c(queryStartTemp))

# store copy of queryStart for swap in vector
...
if(is.null(opt$refIDs)){
  ...
  # if no limit then keep all
  if(is.null(opt$keep_ref)){
    opt$keep_ref = length(chromMax)
  }
  # list of ref names sorted by decreasing ref sizes
...
if (opt$similarity) {
  # add a bit to width to accomodate the legend
  maj_xaxis=1.125
  ...
  # do not add to width when no legend
  maj_xaxis=1
...
# gp with width majorated when legend
ggsave(filename = paste0(opt$output_filename, ".png"), width = maj_xaxis*opt$plot_size, height = opt$plot_size, units = "in", dpi = 300, limitsize = F)
