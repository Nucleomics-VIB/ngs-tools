...
# [optional: -e <qiime2_env|qiime2-amplicon-2025.4>]
# [optional: -h <this help text>]'

while getopts "i:o:t:d:c:v:e:h" opt; do
  case $opt in
    i) infile=${OPTARG};;
    o) outpref=${OPTARG};;
    t) threads=${OPTARG};;
...
echo "# saving Eukaryome classifier information"
info_file="${outpref}_classifier-info.txt"
cat > "${info_file}" << EOT
# Eukaryome Classifier Information
# Generated on $(date)
# by script: $(basename "$0") version ${version}

Classifier: ${eukaryomeClassifier}
Version: ${Version}
Location: ${dbpath}
Input file: ${infile}
...
# Activate conda environment for QIIME 2
echo "# activating the ${qiime_env} conda environment"
eval "$(conda shell.bash hook)"
conda activate ${qiime_env} || { echo "# Error: Failed to activate ${qiime_env} environment"; exit 1; }
