...
usage() {
  ...
  echo "  -o, --output <prefix>   Output file prefix. Output files will be placed in $OUTPUT_DIR and named:"
  echo "                          $OUTPUT_DIR/<prefix>.R1.decontaminated.fastq.gz"
  echo "                          $OUTPUT_DIR/<prefix>.R2.decontaminated.fastq.gz"
  echo ""
  ...
  echo ""
  echo "Example:"
  echo "  $0 -1 read1.fastq.gz -2 read2.fastq.gz -o output -t 8 -i /path/to/index -a bowtie2"
  exit 1
}
